...
\begin{itemize}
   ...
   \item[$\Xi'$]: Multi-set of facts consumed during the whole process (output).
...
The matching judgment uses the form $\mo \Gamma; \Delta; \Xi; \Omega; H; C; R \rightarrow \Xi'; \Delta'; \Gamma'$:
...
If the matching process fails, we use the continuation stack through the $\cont$judgment. Note that we use different continuation frames for persistent and linear facts.
...
If the previous matching process fails, we pick the top frame from the stack $C$ and restore the matching process using another fact and/or context. The continuation judgment $\cont C; P; H; R; \Gamma; \Xi'; \Delta'; \Gamma'$ deals with the backtracking process where the meaning of each argument is as follows:
...
If the matching process fails, we need to backtrack to the previous frame and restore the matching process at that point. The judgment that backtracks has the form $\contc \Gamma; \Delta_N; \Xi_N; \Delta_{N1}; C; AB; \Omega_N \rightarrow \Xi'; \Delta'; \Gamma'$:
...
After a comprehension is matched and before we derive the head of such comprehension, we need to "fix" the continuation stack. In a nutshell, we remove all the frames except the first frame and remove the consumed linear facts from the remaining frames so that they are still valid for the next application of the comprehension.
...
\begin{enumerate}
   ...
   \item[$C$]: Continuation stack for the comprehension.
   \item[$AB$]: Comprehension $\comp A \lolli B$ that is being matched.
   \item[$\Omega_N$]: Ordered list of remaining terms of the head of the rule to be derived.
   \item[$\Delta_N$]: Multi-set of linear facts that were still available after matching the body of the rule.
\end{enumerate}
